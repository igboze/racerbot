...
import type { PnLCard as PnLCardProps } from "@racerbot/shared";
import "./PnLCard.css";

export interface ExtendedPnLCardProps extends PnLCardProps {
  isLive?: boolean;
  className?: string;
...
export function formatPosition(value?: number | null, unit?: string): string {
  if (value === undefined || value === null || isNaN(value)) return "—";
  let out: string;
...
export function BoltIcon({ className, gradient = true }: { className?: string; gradient?: boolean }) {
  return (
